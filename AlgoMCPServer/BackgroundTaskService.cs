...
public class BackgroundTaskService(IConfiguration configuration) : BackgroundService
{
    private readonly String API_KEY = configuration["API_KEY"] ?? throw new ArgumentNullException("API_KEY is not configured.");

    private readonly String API_SECRET = configuration["API_SECRET"] ?? throw new ArgumentNullException("API_SECRET is not configured.");
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            // Your background task logic here
            Console.WriteLine("Background task running...");
            await Task.Delay(60000, stoppingToken); // Delay for 60 seconds
        }
    ...
    private async Task HandleMinuteBar(IBar agg)
    {
        ...
        {
            Console.WriteLine($"Received {agg} Waiting on more data.");
            return;
        }
